...
# Модификация: добавлена поддержка модулей из папки Modules
...
class PrototypeEntry:
    ...
    filepath: str = ""
    file_index: int = 0
    # Новое поле: корневая директория прототипов (основная или модульная)
    prototypes_root: Optional[Path] = None


class ModuleInfo(NamedTuple):
    """Информация о модуле"""
    root: Path                     # Modules/ModuleName
    resources: Path                # Modules/ModuleName/Resources
    prototypes_src: Path           # .../Resources/Prototypes
    locale_en: Path                # .../Resources/Locale/en-US
    locale_ru: Path                # .../Resources/Locale/ru-RU
    prototypes_ru: Path            # .../Resources/Locale/ru-RU/ss14-ru/prototypes
...
class LocalizationManager:
        ...
        if modules_dir.exists():
            for candidate in modules_dir.iterdir():
                if not candidate.is_dir():
                    continue
                module_res = candidate / "Resources"
                if not module_res.exists():
                    continue
                prototypes_src = module_res / "Prototypes"
                locale_en = module_res / "Locale" / "en-US"
                locale_ru = module_res / "Locale" / "ru-RU"
                prototypes_ru = locale_ru / "ss14-ru" / "prototypes"
                self.modules.append(ModuleInfo(
                    root=candidate,
                    resources=module_res,
                    prototypes_src=prototypes_src,
                    locale_en=locale_en,
                    locale_ru=locale_ru,
                    prototypes_ru=prototypes_ru,
                ))
    ...
    # --------------------------------------------------------------------------
    #  Синхронизация FTL (системные файлы локализации)
    # --------------------------------------------------------------------------
    def sync_systems(self) -> None:
        ...
        for module in self.modules:
            if not module.locale_en.exists():
                continue
            # Синхронизируем en-US -> ru-RU для модуля
            valid_module_files = self._sync_directories(
                module.locale_en, module.locale_ru, global_memory
            )
            # При чистке игнорируем папку с прототипами модуля, чтобы не удалить сгенерированные файлы
            ignore_module_dirs = [module.prototypes_ru] if module.prototypes_ru.exists() else []
            self._cleanup_orphans(module.locale_ru, valid_module_files, ignore_dirs=ignore_module_dirs)

    def _load_translation_memory(self) -> Dict[str, FtlEntry]:
        """Собирает все существующие переводы из всех ru-RU директорий (основной + модули)"""
        memory: Dict[str, FtlEntry] = {}
        ru_dirs = [self.ru_ru_dir, self.robust_ru_dir]
        ru_dirs.extend(m.locale_ru for m in self.modules if m.locale_ru.exists())
        for target_dir in ru_dirs:
    ...
    def process_prototypes(self) -> None:
        ...
        # Собираем все YAML файлы: основные + из модулей
        yaml_files: List[Path] = []
        if self.prototypes_src_dir.exists():
            yaml_files.extend(self.prototypes_src_dir.rglob("*.yml"))
        for module in self.modules:
            if module.prototypes_src.exists():
                yaml_files.extend(module.prototypes_src.rglob("*.yml"))

        prototypes: Dict[str, PrototypeEntry] = {}

        with ProcessPoolExecutor() as executor:
            futures = {
                executor.submit(self._parse_yaml, yml_file, self._get_prototypes_root(yml_file)): yml_file
        ...
        # Чистим основные и модульные ru-prototyes папки
        self._cleanup_orphans(self.prototypes_ru_dir, valid_ftls)
        for module in self.modules:
            if module.prototypes_ru.exists():
                self._cleanup_orphans(module.prototypes_ru, valid_ftls)

        logging.info("Обработка прототипов завершена.")

    def _get_prototypes_root(self, filepath: Path) -> Path:
        """Определяет, какому корню прототипов принадлежит файл (основному или модульному)"""
        if filepath.is_relative_to(self.prototypes_src_dir):
            return self.prototypes_src_dir
        for module in self.modules:
            if filepath.is_relative_to(module.prototypes_src):
                return module.prototypes_src
        # fallback: родительская директория
        return filepath.parent

    @staticmethod
    def _parse_yaml(filepath: Path, prototypes_root: Path) -> List[PrototypeEntry]:
    ...
    def _resolve_inheritance(
        ...
        def resolve(proto_id: str, visited: Set[str]) -> PrototypeEntry:
            ...
            resolved_proto = PrototypeEntry(
                ...
                prototypes_root=proto.prototypes_root,
            )
    ...
    def _generate_prototype_ftls(
        ...
        # 1. Собираем внешние ключи из всей локализации (основной + модули)
        external_keys: Set[str] = set()
        ru_locale_dirs = [self.ru_ru_dir]
        ru_locale_dirs.extend(m.locale_ru for m in self.modules if m.locale_ru.exists())
        for ru_dir in ru_locale_dirs:
            for ftl_file in ru_dir.rglob("*.ftl"):
                if ftl_file.is_relative_to(self.prototypes_ru_dir):
                    continue
                # Исключаем папки прототипов модулей
                skip = False
                for module in self.modules:
                    if ftl_file.is_relative_to(module.prototypes_ru):
                        skip = True
                        break
                if skip:
                    continue
        ...
        # 2. Загружаем существующие ftl-записи из всех ru-папок (включая prototypes)
        ...
        # 3. Определяем целевые директории для прототипов
        #    Основная и для каждого модуля
        proto_target_dirs: Dict[Path, Path] = {
            self.prototypes_src_dir: self.prototypes_ru_dir
        }
        for module in self.modules:
            if module.prototypes_src.exists():
                proto_target_dirs[module.prototypes_src] = module.prototypes_ru
        ...
        for proto in prototypes.values():
            ...
            # Определяем, из какого корня прототипов загружен файл
            proto_root = proto.prototypes_root
            if proto_root is None:
                # fallback: пытаемся угадать по filepath
                proto_root = self._get_prototypes_root(Path(proto.filepath))
            target_base = proto_target_dirs.get(proto_root)
            if target_base is None:
                # Если корень не найден (например, сторонний прототип) – пропускаем
                continue

            try:
                rel_path = Path(proto.filepath).relative_to(proto_root)
                lower_parts = [p.lower() for p in rel_path.parts]
                ftl_path = target_base.joinpath(*lower_parts).with_suffix(".ftl")
            except ValueError:
                continue
        ...
        for key, (entry, old_path) in global_entries.items():
            if key not in used_keys and key not in external_keys:
                # Проверяем, не лежит ли этот файл в директории прототипов (основной или модульной)
                is_proto_file = False
                for target_dir in proto_target_dirs.values():
                    if old_path.is_relative_to(target_dir):
                        is_proto_file = True
                        break
                if is_proto_file:
                    # Файл прототипов, который больше не нужен – удалим позже через cleanup
                    continue
    ...
    # --------------------------------------------------------------------------
    #  Расстановка тире и проверка непереведённых строк
    # --------------------------------------------------------------------------
    def fix_dashes(self) -> None:
        ...
        search_dirs.extend(m.locale_ru for m in self.modules if m.locale_ru.exists())
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Инструмент управления локализацией SS14 (с поддержкой модулей)."
    )
    parser.add_argument(
        "--root",
        type=str,
